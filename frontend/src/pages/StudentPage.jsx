...
useEffect(() => {
  ...
  const handleOcrImageSelect = (e) => {
    ...
    if (file) {
      ...
      if (!validTypes.includes(file.type)) {
        setOcrError('Invalid file type. Please upload a JPG, PNG, BMP, or TIFF image.');
      ...
      if (file.size > maxSize) {
        setOcrError(`File size exceeds 10MB limit. File size: ${(file.size / (1024 * 1024)).toFixed(2)}MB`);
        if (fileInputRef.current) fileInputRef.current.value = null;
        return;
      }
  ...
  const handleOcrUploadWithFile = async (file) => {
    ...
    try {
      ...
      const errorMessage = error.response?.data?.detail || error.message || 'Failed to process image.';
  ...
  const handleExportToCSV = (table, index) => {
    ...
    const studentName = table.rows[0] && table.rows[0]['Student Name'] ? table.rows[0]['Student Name'] : '';
    
    // Filter out Student Name and Register Number columns from headers
    const allHeaders = table.headers || Object.keys(table.rows[0]);
    const headers = allHeaders.filter(h => h !== 'Student Name' && h !== 'Register Number' && h !== 'Assessment Date').map(h => h.replace(/^Session\s+/i, ''));
    ...
    table.rows.forEach(row => {
      const values = allHeaders
        .filter(h => h !== 'Student Name' && h !== 'Register Number' && h !== 'Assessment Date')
        .map(header => {
    ...
    link.setAttribute('download', `table_${index + 1}_${new Date().getTime()}.csv`);
                    ...
                    <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                      ...
                      <div className="grid grid-cols-1 gap-3">
                        <div className="bg-white rounded p-3">
                          <p className="text-sm font-medium text-gray-700">Skills Found: {extractionSummary.total_skills_found}</p>
                          <p className="text-xs text-gray-600 mt-1">{extractionSummary.skills_found?.join(', ')}</p>
                        </div>
                        {extractionSummary.sample_values && Object.keys(extractionSummary.sample_values).length > 0 && (
                          <div className="bg-white rounded p-3">
                            <p className="text-sm font-medium text-gray-700 mb-2">Sample Values (First 10 sessions):</p>
                            <div className="space-y-1">
                              {Object.entries(extractionSummary.sample_values).map(([skill, values]) => (
                                <div key={skill} className="text-xs font-mono">
                                  <span className="font-semibold text-[#E38B52]">{skill}:</span>
                                  <span className="ml-2 text-gray-700">[{values.join(', ')}]</span>
                                </div>
                              ))}
                            </div>
                          </div>
