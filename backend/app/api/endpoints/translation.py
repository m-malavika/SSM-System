"""
Translation endpoint using IndicTrans2 for Indian languages
"""
...
async def clear_translation_cache(current_user: User = Depends(get_current_user)):
    ...
    old_type = "IndicTrans2" if not isinstance(_translation_model, dict) and _translation_model is not None else "Helsinki" if _translation_model else "None"
...
def get_translation_model():
    """
    Lazy load translation model - uses Helsinki-NLP for reliability
    """
    global _translation_model, _tokenizer
    
    if _translation_model is None:
        # Use reliable Helsinki-NLP models
        logger.info("Using Helsinki-NLP models for translation")
        _translation_model = {}
        _tokenizer = {}
...
def get_model_for_language(target_lang):
    """Get or load the appropriate model for the target language"""
...
async def translate_text(
    ...
    try:
        ...
        # Map IndicTrans2 language codes to ISO codes for Helsinki model
        language_map = {
        ...
        model_dict, tokenizer_dict, model_type = get_translation_model()
        ...
        with torch.no_grad():
            outputs = model.generate(
                **inputs,
                max_length=512,  # Increased output limit
                num_beams=5,
                early_stopping=True,
        ...
        logger.info(f"Translation output: {len(translated_text)} characters")
